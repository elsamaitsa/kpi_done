...
if ($this->session->userdata('level_user') == 'Agent' || $this->session->userdata('level_user') == 'Member') {
  $id = 'a.id_member';
  $query = 'AND a.id_keyaccount IS NULL';
} else {
  $id = 'a.id_keyaccount';
  $query = '';
}
...
//Keranjang
$notif_pesan = $this->db->query("SELECT id,title,message,tipe,node,status,created_at FROM pesan_notifikasi WHERE status='unread' AND id_member='$id_member' LIMIT 10");
...
AND a.status = 'uncheckout' GROUP BY a.id");



// Notifikasi

$retailProcess = $this->db->query("SELECT a.id as id_orders, d.nama_kategori, c.total_quantity, a.status_menunggu_konfirmasi, a.status_pesanan_diproses, a.status_pesanan_dikirim, a.status_sampai_tujuan
  FROM orders_retail a 
  LEFT JOIN bank_retail b ON b.id = a.id_bank_retail
  LEFT JOIN orders_retail_produk c ON c.id_orders_retail = a.id
  LEFT JOIN produk_kategori d ON d.id = c.id_produk_kategori
  LEFT JOIN produk_kategori_foto e ON e.id_produk_kategori = d.id
  LEFT JOIN invoice_orders_retail f ON f.id_orders_retail = a.id
  WHERE a.id_member = '$id_member'
  GROUP BY a.id 
  ORDER BY a.created_at DESC
");

$orderProcessCOS = $this->db->query("SELECT f.id as id_produk, a.id as id_ra, d.id_shipping_instruction, a.id_member, a.no_cos, a.id_keyaccount, e.nama, e.level_user, a.status as status_ra, a.jenis_produk, a.ra_from, a.status_document as status_document_ra, b.total_po, c.total_pi, d.total_invoice
			FROM request_allocation a
			LEFT JOIN (SELECT updated_at, id_request_allocation, COUNT(id) as total_po FROM purchase_order 
			WHERE status = 'PO'
			...
			LEFT JOIN (SELECT a.id_shipping_instruction, c.id_request_allocation, COUNT(a.id) as total_invoice, a.transfer_date
			...
			LEFT JOIN produk_kategori f ON a.jenis_produk = f.nama_kategori
			WHERE $id = '$id_member'
			AND MONTH(a.periode_start) = '$month'
			AND YEAR(a.periode_start) = '$year'
			AND MONTH(a.periode_end) = '$month'
			AND YEAR(a.periode_end) = '$year'" . $query . "
      ORDER BY a.updated_at DESC, b.updated_at DESC, d.transfer_date DESC
      ");
$orderProcessRA = $this->db->query("SELECT f.id as id_produk, a.id as id_ra, d.id_shipping_instruction, a.id_member, a.no_cos, a.id_keyaccount, e.nama, e.level_user, a.status as status_ra, a.jenis_produk, a.ra_from, a.status_document as status_document_ra, b.total_po, c.total_pi, d.total_invoice
			FROM request_allocation a
...
$total_RA = count($output) + count($retailProcess->result_array());

 $ch = curl_init();
 $url = "https://dilan-reporting.perhutani.co.id/api/master-country";
 $url = "https://dilan-reporting.perhutani.co.id/api/master-country";
 $header = array(
   "Authorization: Bearer 183|7oWGxIrgcRLFYl5yVRXXH72Yqd06Mnv2X7FHGVDF",
  ...
  <div id="page">
              ...
              <div class="row float-right" style="display:flex; position: relative;">
                ...
                ?>
                  <div class="d-flex">
                      ...
                      <div class="dropdown block-company-wrapper">
                        ...
                        <ul class="dropdown-menu content-dropdown">
                          <li class="header-dropdown" style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <a href="<?= base_url(); ?>keranjang" class="fw-6 float-right pr-0" style="color: #219653; font-size: 14px;">
                              Lihat Keranjang
                            ...
                            <?php if ($keranjang->num_rows() > 0) { ?>
                              <?php foreach ($keranjang->result_array() as $result) { ?>
                                <div class="d-flex mb-1">
                                  <!-- GANTI DENGAN GAMBAR -->
                                  <?php if ($result['url_foto'] != "") { ?>
                                    <img class="item-center mr-2" style="width: 85px; height: 85px;" src="<?= base_url(); ?>product_retail/<?= $result['url_foto'] ?>"></img>
                                  <?php } else { ?>
                                    <img class="item-center mr-2" style="width: 85px; height: 85px;" src="assets/images/products/default.png"></img>
                                  <?php } ?>
                        ...
                        <ul class="dropdown-menu content-dropdown notifikasi">
                          <li class="header-dropdown">
                            <div style="display: flex; justify-content:space-between">
                              <div style="display: flex; align-items: center;">
                                ...
                                <span class="fs-12 fw-7" style="color: white ; background-color: #EB5757; border-radius: 999px; padding: 4px 9px;"><?= count($output) + count($retailProcess->result_array()); ?></span>
                              </div>
                              <a href="<?= base_url(); ?>notifikasi" class="fw-6 float-right pr-0" style="color: #219653; font-size: 14px;">
                                Lihat Notifikasi
                                <img src="<?= base_url(); ?>/assets/images/entypo_chevron-right.png" height="16px" alt="">
                              </a>
                            </div>
                          ...
                          <li class="items-dropdown ">
                            <div class="notif-retail" style="width: 568px;">
                              <?php foreach ($retailProcess->result_array() as $key => $resultRetail) { ?>
                                <?php if ($key < 2) { ?>
                                  <div class="box bg-gray2 mt-2">
                                    <div class="box-body" style="padding: 10px 15px;">
                                      <div class="row">

                                        <div class="col-sm-5 col-md-5">
                                          <div class="v-middle">
                                            <h5 class="font-weight-bold mr-2"><?php echo $resultRetail['nama_kategori'] ?></h5>
                                            <p class="font-10 font-weight-thick text-muted"><?php echo $resultRetail['total_quantity'] ?> Kg</p>
                                          </div>

                                          <?php if ($resultRetail['status_sampai_tujuan'] != '') { ?>
                                            <div class="v-middle">
                                              <i class="icon-notif-pending mr-1"></i>
                                              <h6 class="text-main">Pesanan Telah Sampai</h6>
                                            </div>

                                          <?php } else if ($resultRetail['status_pesanan_dikirim'] != '') { ?>
                                            <div class="v-middle">
                                              <i class="icon-notif-pending mr-1"></i>
                                              <h6 class="text-main">Pesanan Sedang Dikirim</h6>
                                            </div>

                                          <?php } else if ($resultRetail['status_pesanan_diproses'] != '') { ?>
                                            <div class="v-middle">
                                              <i class="icon-notif-pending mr-1"></i>
                                              <h6 class="text-main">Pesanan Sedang Diproses</h6>
                                            </div>
                                          <?php } else if ($resultRetail['status_menunggu_konfirmasi'] != '') { ?>
                                            <div class="v-middle">
                                              <i class="icon-notif-pending mr-1"></i>
                                              <h6 class="text-main">Menunggu Konfirmasi</h6>
                                            </div>
                                          <?php } else if ($resultRetail['status_menunggu_konfirmasi'] == '') { ?>
                                            <a class="v-middle" href="<?php echo base_url(); ?>list_pembayaran">
                                              <i class="icon-notif-pending mr-1"></i>
                                              <h6 class="text-main">Menunggu Proses Pembayaran</h6>
                                            </a>
                                          <?php } ?>
                                        </div>


                                        <div class="col-sm-7 col-md-7 pd-0">
                                          <ul id="progressbar_1" class="mg-0">
                                            <?php if ($resultRetail['status_menunggu_konfirmasi'] == '') { ?>
                                              <li>Menunggu Konfirmasi</li>
                                            <?php } else { ?>
                                              <li class="active">Menunggu Konfirmasi</li>
                                            <?php } ?>
                                            <?php if ($resultRetail['status_pesanan_diproses'] == '') { ?>
                                              <li>Pesanan Diproses</li>
                                            <?php } else { ?>
                                              <li class="active">Pesanan Diproses</li>
                                            <?php } ?>
                                            <?php if ($resultRetail['status_pesanan_dikirim'] == '') { ?>
                                              <li>Pesanan Dikirim</li>
                                            <?php } else { ?>
                                              <li class="active">Pesanan Dikirim</li>
                                            <?php } ?>
                                            <?php if ($resultRetail['status_sampai_tujuan'] == '') { ?>
                                              <li>Sampai Tujuan</li>
                                            <?php } else { ?>
                                              <li class="active">Sampai Tujuan</li>
                                            <?php } ?>
                                ...
                                <div class="mt-1  mb-1">
                                  <div class="d-flex" style="justify-content: space-between; align-items: center;">
                                    <div class="col-sm-7 pl-0">
                                      ...
                                      <?php } else { ?>
                                        <div class="label-green">Domestik</div>
                                      <?php } ?>
                                      <h5 class="fs-14 fw-6 mr-1">Pembelian <?php echo $result['jenis_produk'] ?></h5>
                                    </div>
                                    <a href="<?= base_url(); ?>detail_produk?id=<?= bin2hex(encrypt($result['id_produk'])); ?>" class="fw-6 fs-14 float-right" style="color: #219653;">
                                      Lihat Detail
                          ...
                          <?php if ($total_RA > 4) { ?>
                            <li class=" dropdwon-items item-center">
                              <a class="fs-12 fw-6 mb-1" href="<?= base_url(); ?>notifikasi" style="color: #4F4F4F;"><?= ($total_RA - 4); ?> more</a>
                            </li>
                          <?php } ?>
                        </ul>
                      </div>
                    ...
                    <div class="mr-3">
                      <div class="dropdown block-company-wrapper hidden-xs">
                        <a role="button" data-toggle="dropdown" data-target="#" class="block-company dropdown-toggle icon-menu" href="#"><i class="email_icon" style="color:white;"></i>
                        </a>
                            ...
                            <div class="scrollbar" id="style-1">
                              <div class="force-overflow">
                                <?php foreach ($notif_pesan->result_array() as $result) {
                                  if ($result['tipe'] == "ra") {
                                    $node = "stok?id=" . $result['node'];
                                  } elseif ($result['tipe'] == "po") {
                                    $node = "purchase_order?id=" . $result['node'];
                                  } elseif ($result['tipe'] == "si") {
                                    $node = "proforma_invoice?id=" . $result['node'];
                                  }
                                  $id = $result['id'];
                                  $web = base_url() . $node;
                                ?>
                                  <li class="items-dropdown ">
                                    <div class="popup-content mb-1 pl-2" style="align-items: center;">
                                      ...
                                      <a href="#" onclick="DelData('<?= $id; ?>','<?= $web; ?>')">
                                        <img src="<?= base_url(); ?>assets/images/ic_message_enabled.png" style="width: 30px; margin-right: 13px;" alt=""> </a>
                                      <div style="display: flex; flex-direction: column;">
                                        <h6 style="font-size: 10px; color:#BDBDBD; margin-top: 3px;"><?= $result['created_at']; ?></h6>
                                        <h5 class="font-12 fw-7" style="margin-top: 5px; margin-bottom:2px; color: #444444;"><?= $result['title']; ?></h5>
                                        <p class="font-10 fw-4" style="color: #989898;">
                                          <?= $result['message']; ?>
                                        </p>
                                      </div>
                                    </div>
                    ...
                    <div class="mr-1">
                      <div class="dropdown block-company-wrapper">
                        ...
                        <ul class="dropdown-menu content-dropdown" style="min-width: 300px; margin:10px -268px;">
                          ...
                          <!-- <li class="font-13 mb-1">
                            <a href="<?= base_url(); ?>history_transaksi">
                              <img src="<?= base_url(); ?>/assets/images/ic_history.png" style="width: 20px;" class="mr-2" alt="">
                              History Pesanan Retail
                            </a>
                          </li> -->
                          <li class="font-13 mb-1">
              ...
              <ul>
                <li class="mt-root">
                  <div class="mt-root-item"><a href="<?php echo base_url(); ?>">
                      <div class="title title_font"><span class="title-text"><?php echo $this->lang->line('menu_beranda'); ?></span></div>
                    </a></div>
                </li>
                <li class="mt-root">
                  <div class="mt-root-item"><a href="<?php echo base_url(); ?>panduan">
                      <div class="title title_font"><span class="title-text"><?php echo $this->lang->line('menu_panduan'); ?></span></div>
                    </a></div>
                </li>
                <li class="mt-root">
                  <div class="mt-root-item"><a href="<?php echo base_url(); ?>article/static_art/cara-pembayaran">
                      <div class="title title_font"><span class="title-text"><?php echo $this->lang->line('menu_pembayaran'); ?></span></div>
                    </a></div>
                </li>
                <li class="mt-root">
                  <div class="mt-root-item"><a href="<?php echo base_url(); ?>article/static_art/syarat-dan-ketentuan">
                      <div class="title title_font"><span class="title-text"><?php echo $this->lang->line('menu_syarat_ketentuan'); ?></span></div>
                    </a></div>
                </li>
                <li class="mt-root">
                  <div class="mt-root-item"><a href="<?php echo base_url(); ?>SertifikatPerhutani">
                      <div class="title title_font"><span class="title-text"><?php echo $this->lang->line('menu_sertifikat_pethutani'); ?></span></div>
                    </a></div>
                </li>
                <li class="mt-root">
                  <div class="dropdown jtv-language-box mt-2">
                    ...
                    <a role="button" data-toggle="dropdown" data-target="#" class="block-language dropdown-toggle d-flex" href="#">
                      <i class="<?= ($this->session->userdata('site_lang') == 'indo') ? 'ina-flag p-absolute' : 'uk-flag p-absolute' ?>"></i><span class=" fa fa-chevron-down lay-flag"></span> </a>
                    <ul class="dropdown-menu" role="menu">
                      <li> <a class="<?php if ($this->session->userdata('site_lang') == 'indo' && $this->session->userdata('asal_negara') == 'Indonesia') echo 'selected="selected"'; ?>" href="<?php echo base_url(); ?>LanguageSwitcher/switchLang/indo" style="display:flex;"> <i class="ina-flag p-absolute"></i><span class="ml-3"> Indonesia</span></a> </li>
                      <li> <a class="<?php if ($this->session->userdata('site_lang') == 'english' && $this->session->userdata('asal_negara') != 'Indonesia') echo 'selected="selected"'; ?>" href="<?php echo base_url(); ?>LanguageSwitcher/switchLang/english" style="display:flex;"> <i class="uk-flag p-absolute"></i><span class="ml-3"> English</span></a> </li>
                    </ul>
                  </div>
                </li>
                <li class="mt-root">
   ...
   <!-- modal login -->
    ...
    <div class="modal-dialog" role="document">
      <form id="form-login" name="formAddress" method="POST" action="" class="modal-content2">
        ...
        <div class="modal-title2" style="margin-bottom:25px;">
          <span><img alt="e-commerce" src="<?php echo base_url(); ?>assets/images/tokoperhutani.png" style="width: 55%; margin-top: 10%; margin-left: 20%;"></span>
          <a class="modal-close" href="#" id="btn_close"><img src="assets/images/eva_close-fill.png" alt="..."></a>
        </div>
  
        <div class="row">
          <div class="col-md-12 col-lg-12 col-xs-12 mb-1">
            <span style="font-weight: 600;font-size: 16px;">User ID</span>
          </div>
          <div class="col-md-12 col-lg-12 col-xs-12 input-group" style="margin-bottom:1px;">
        ...
        <div class="row mt-1">
          <div class="col-md-12 col-lg-12 col-xs-12 mb-1">
            <span style="font-weight: 600;font-size: 16px;">Password</span>
          </div>
          <div class="col-md-12 col-lg-12 col-xs-12 input-group" style="margin-bottom:1px;">
        ...
        <div class="row mt-1">
          ...
          <div class="col-md-12 col-lg-12 col-xs-12 mb-1" style="text-align: right;">
            <a href="<?php echo base_url(); ?>forgot_password">Lupa kata sandi ? </a>
          </div>
        </div>
        <button class="modal-btn" id="btn_login" type="button">
          <span>Login</span>
        </button>
        <span class="modal-footer" id="linkRegister">Don’t have an account? <a href="#">Sign Up</a></span>
      </form>
      </div>
    </div>
    <!-- end modal login -->
          ...
          <form id="form-login" name="formAddress" method="POST" action="">
            ...
            <div class="modal-footer" style="border-top:0px;">
              ...
              <div class="text-center" style="margin: 33px 0px;">
                <h6 class="text-muted font-weight-normal" id="linkRegister">Belum punya akun ? <a class="text-main font-weight-bold" href="#">Daftar</a></h6>
              </div>
            </div>
          </form>
        </div>
     ...
     <div class="fade modal-container" id="register" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <form class="modal-content1">
        ...
        <div class="row mb-1">
          ...
          <div class="col-md-12 col-lg-12 col-xs-12">
            <select class="input-custom form-control" id="level_user" name="level_user"> 
        ...
        <span class="modal-footer" id="Link_login">Have an account? <a href="#">Log In</a></span>
      </form>
    </div>
    <!-- end modal register -->
    ...
    <script type="text/javascript">
      function DelData(id, OpenWeb) {
        param = "q=" + id;
        var xmlhttp = new XMLHttpRequest();

        xmlhttp.open("POST", "<?php echo base_url(); ?>auth/ReadNotif", true);
      ...
      function showPassword() {
        ...
        var myCallBack = function() {
          recaptcha1 = grecaptcha.render('recaptcha1', {
            'sitekey' : '6LdXfuYUAAAAAC2J8Hfrxue7hQ-0SIc7w8aaXa0s'
          });
    ...
    <script type="text/javascript">
      ...
      $('#btn_register').on('click', function() {
        ...
        var kode_negara = $("#asal_negara").val();
        var level_user = 'Member';
        var tokenCode = '<?php echo $this->security->get_csrf_hash(); ?>';
        var asal_negara = $('select[name=asal_negara]').find(':selected').text();
        if (email_register == '') {
            alert("Harap isi data Email terlebih dahulu!");
            return false;
        } 

        if (kode_negara == '') {
            alert("Harap isi data Negara terlebih dahulu!");
            return false;
        }        
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            //$("#csrf_header").val(data.POTPtokenCode);
            if (data.status == "ok") {
              var url = "<?php echo base_url();?>personal_data/register_sukses";
